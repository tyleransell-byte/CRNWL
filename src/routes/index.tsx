...
import { ArrowRight, Search, Ship, UtensilsCrossed, Waves } from "lucide-react";
...
import { CATEGORIES, CORNWALL_LOCATIONS } from "@/lib/jobs-data";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Work in CRNWL — Cornwall Hospitality Jobs" },
      {
        property: "og:description",
        content: "Cornwall's dedicated hospitality jobs board for candidates and employers.",
      },
    ],
  }),
  component: Home,
  ...
  return (
    <>
      <section className="relative isolate overflow-hidden">
        ...
        <div className="relative mx-auto w-full max-w-6xl px-4 py-24 sm:py-32">
          <p className="inline-flex items-center gap-2 rounded-full border border-primary-foreground/25 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-primary-foreground/85">
            <Waves className="size-3.5" /> Kernow hospitality
          </p>
          ...
          <p className="mt-5 max-w-xl text-lg text-primary-foreground/85">
            Season-long kitchen gigs, year-round hotel roles and harbourside bar shifts — posted
            direct by Cornish employers.
          </p>
          ...
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              navigate({ to: "/jobs", search: { q: q || undefined } });
            }}
          >
            ...
            <Button type="submit" size="lg" variant="hero" className="h-12">
              Search jobs
            </Button>
          </form>

          <div className="mt-6 flex flex-wrap gap-2">
            {CORNWALL_LOCATIONS.slice(0, 6).map((loc) => (
              <Link
                key={loc}
                to="/jobs"
                search={{ location: loc }}
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-14">
        <h2 className="font-display text-2xl font-bold">Browse by trade</h2>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 pb-4">
        <div className="flex items-end justify-between">
          <h2 className="font-display text-2xl font-bold">Latest roles</h2>
          ...
          {!isLoading && jobs?.length === 0 && (
            <div className="rounded-xl border border-dashed border-border p-10 text-center md:col-span-2">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Be the first Cornish employer on the board.
              </p>
        ...
        <div className="rounded-2xl border border-border bg-sand p-8 sm:p-12">
          <div className="flex flex-col items-start gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="max-w-xl">
              ...
              <h2 className="mt-3 font-display text-2xl font-bold">Hiring for the season?</h2>
              <p className="mt-2 text-muted-foreground">
                Create an employer account, post your role in a couple of minutes and manage
                applicants in one dashboard. No agency fees.
              </p>
            </div>
