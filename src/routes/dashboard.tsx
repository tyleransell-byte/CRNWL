...
function EmployerView({ userId }: { userId: string }) {
  ...
  const { data: membership, isLoading: membershipLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select(
          "membership_status, membership_expires_at, stripe_customer_id, stripe_subscription_id",
        )
        .eq("id", userId)
        .single();
  ...
  const { data: jobs, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("jobs")
        .select(
          "*, applications(id, full_name, email, phone, cover_note, status, created_at)",
        )
  ...
  const expiryDate = membership?.membership_expires_at
    ? new Date(membership.membership_expires_at).toLocaleDateString("en-GB", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })
        ...
        <div className="rounded-2xl border border-border bg-sand p-6 sm:p-8">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Join CRNWL as a Founding Employer and advertise as many
                hospitality vacancies as your business needs for the next
                12 months.
              </p>
            </div>
...
function CandidateView({ userId }: { userId: string }) {
  const { data: applications, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("applications")
        .select(
          "id, status, created_at, jobs(id, title, company_name, location)",
        )
  ...
  if (isLoading) {
    return <Skeleton className="mt-8 h-40 w-full" />;
  }
